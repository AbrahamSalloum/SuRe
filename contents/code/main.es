...
//find the most recent comic
   ...
   if (id == comic.Page) {         
	var re = new RegExp("<img src='(http://sufficientlyremarkable.com/images/comics/[^\"]+)' class=\"comic\" alt=\'([^\"]+)\' title=\'([^\"]+)\'>", "i");
        var match = re.exec(data);
        if (match != null) {  	
	  comic.title=match[3];
...
//previous identifier
        var re = new RegExp("<a class=\"comicPagination nav-prev\" href=\"http://sufficientlyremarkable.com/([^\"]+)\">", "i");
        var match = re.exec(data);
        if (match != null) {
...
//next identifier
        var re = new RegExp("<a class=\"comicPagination nav-next\" href=\"http://sufficientlyremarkable.com/([^\"]+)\">", "i");
        var match = re.exec(data);
        if (match != null) {
